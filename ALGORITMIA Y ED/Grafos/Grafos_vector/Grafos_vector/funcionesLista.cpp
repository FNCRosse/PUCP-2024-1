...
ciudad retornaCabeza(struct Lista tad){
    return tad.cabeza->dato;
}

/*inserta un nodo siempre al inicio de la lista*/
void insertarAlInicio(struct Lista & tad, ciudad elemento){
...
struct Nodo * crearNodo(ciudad elemento, struct Nodo * siguiente){
    struct Nodo * nuevoNodo = new struct Nodo;
...
void imprime(const struct Lista & tad){
    ...
    if (esListaVacia(tad)){
        ...
        while(recorrido != nullptr){
            ...
            cout<<recorrido->dato.GetDestino()<<" "<<recorrido->dato.GetDistancia()<<" ";        
            recorrido = recorrido->siguiente;
        }
        cout<<"]"<<endl;
    }
