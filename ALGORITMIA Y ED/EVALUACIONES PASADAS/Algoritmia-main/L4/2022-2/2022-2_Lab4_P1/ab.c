...
void imprime_raiz(ArbolBinario tad){
    printf("%d %d", tad->elemento.cromo,tad->elemento.numeroArr);
}
...
ArbolBinario especular(ArbolBinario tad){
    ArbolBinario arbol_especular;
    if (es_arbol_vacio(tad))
        construir_arbol_binario(&arbol_especular);
    else{
        ArbolBinario hijo_izq=especular(tad->hijo_der);
        ArbolBinario hijo_der=especular(tad->hijo_izq);
        plantar_arbol_binario(&arbol_especular, hijo_izq, tad->elemento, hijo_der);
    }
    return arbol_especular;
}

ArbolBinario copia_arbol_binario(ArbolBinario tad){
    ArbolBinario arbol_copia;
    if (es_arbol_vacio(tad))
        construir_arbol_binario(&arbol_copia);
    else{
        ArbolBinario hijo_izq=copia_arbol_binario(tad->hijo_izq);
        ArbolBinario hijo_der=copia_arbol_binario(tad->hijo_der);
        plantar_arbol_binario(&arbol_copia, hijo_izq, tad->elemento, hijo_der);
    }
    return arbol_copia;
}
...
void es_zurdo(ArbolBinario tad, int *es_zurdo_arbol, int *numero_descendientes){
    if (es_arbol_vacio(tad)){
        ...
        *numero_descendientes=1;
        ...
        int es_zurdo_hijo_izq, es_zurdo_hijo_der, numero_descendientes_izq, numero_descendientes_der;
